...
also shows a few reserved `meta` keys on the thread as chips: `host`, `cwd` (its hover text names `project`, the directory the session was started in, when the two differ),
