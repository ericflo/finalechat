...
export function ConnectorsCard() {
  ...
  return <><div className="section-title">Integration settings</div><div className="card connector-list"><p>Pair a connector from your integration to control its settings here. Session archives remain available after a connector goes away.</p>{connectors.filter((c) => c.state !== "revoked").map((c) => <ConnectorRow key={c.id} connector={c} refresh={refresh} />)}{!connectors.some((c) => c.state !== "revoked") && <p>No connectors paired yet.</p>}{error && <p role="alert" className="artifact-error">{error}</p>}</div></>;
}
