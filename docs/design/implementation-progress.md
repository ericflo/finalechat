...
- Finish eagent's richer configuration surface: shared prompt/bundle writers and typed resource actions are committed; shared route-test semantics now pass targeted race checks. Conditional undo and a usable custom UI with the current local editor's breadth remain.
- Finish archive provenance, eagent publication recovery on stale/deleted remote parents, and viewer/runtime asset build synchronization. Python publishers now reconcile lost acknowledgements, recapture on competing parents without rewinding sources, and pause after deletion until an explicit single-session recreation. Verified source recovery into a new directory is committed in eagent.
