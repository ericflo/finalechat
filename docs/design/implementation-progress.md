...
- Finish archive provenance. Eagent's isolated worktree now matches the Python publisher's recovery behavior: lost acknowledgements reuse immutable capture identities, competing parents require a fresh capture that preserves both source prefixes, and deletion pauses publication until explicit single-session recreation. Tests cover truncated/divergent histories and retention of pending evidence. Renderer fingerprints participate in publication and Makefile build/test/install/release targets rebuild the matching Go replay assets; the generator has a verification mode. Verified source recovery into a new directory is committed in eagent.
