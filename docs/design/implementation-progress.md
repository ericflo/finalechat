...
- Finish eagent's richer configuration surface: shared prompt/bundle writers and typed resource actions are committed; shared route-test semantics now pass targeted race checks. Conditional undo now passes configuration/resource recovery tests and a trusted browser review check in the isolated eagent worktree. A usable custom UI with the current local editor's breadth remains.
