...
Open **Settings in the conversation header** to change an agent’s models and behavior without leaving the thread. Your account’s integration connects automatically; there is no second pairing approval. Eagent opens its compact Models / Behavior / Prompts / Advanced editor. Claude Code and Codex use focused controls for their declared native settings. Edit a value and tap **Save** once. A local companion publishes typed snapshots and receives durable commands over outbound HTTPS. A publisher token cannot submit human settings commands, and the iframe receives neither token nor general API access.
...
Publish `snapshot.details.thread_external_ids` (or a single `thread_external_id`) to associate a settings resource with existing conversations owned by the same account. `GET /threads/{ref}/settings` includes those resources and any current bound website. Settings-only websites use the `agent-settings` artifact key and do not appear among transcript archive buttons. Eagent publishes its HTML and bounded settings context independently of opt-in native JSONL backups.

In the thread, `settings.read` includes `presentation: "thread"`. Custom editors should stage valid changes as the form changes and clear obsolete proposals, leaving the host’s single Save button to submit. Opening and refreshing never submit a command. The host pins an open website while updates are published, and preserves newer drafts through late acknowledgements. Saved defaults are labeled separately from runtime controls.

For the cross-repository browser test, run eagent’s `go run ./scripts/browser-fixture NEW_TEMP_DIRECTORY`, then in FinaleChat run `EAGENT_SETTINGS_FIXTURE=NEW_TEMP_DIRECTORY/archive FINALECHAT_PLAYWRIGHT_MODULE=/path/to/playwright/index.mjs node scripts/test-thread-settings-browser.mjs`. It uses synthetic data and checks the real thread, iframe editor, one Save, and phone/desktop layout without a user account.
