...
endpoint("POST","/connectors","Connect an owned installation and receive a scoped credential",record({"name":string,"provider":string,"requested_grants":array(ref("SettingsGrant")),"connect":{"type":"boolean"}},["name","provider","requested_grants"]),status="201",description="An account-authenticated integration sets connect=true to activate its own declared settings immediately. Omitting connect preserves legacy pending pairing. Returns connector and one-time scoped secret; only a signed-in human can submit settings commands.")
endpoint("POST","/connectors/{connector}/connect","Connect an existing owned installation",record({"secret":string},["secret"]),description="Requires account authentication and the locally held connector secret. Activates pending installations, preserves active grants, and never reactivates a revoked installation.")
endpoint("GET","/threads/{thread}/settings","List agent settings available in this conversation",description="Returns resources linked by an owned current settings website or published thread_external_id/thread_external_ids hints. Each item includes id, label, scope, provider, available and optional artifact_id/revision_id. Does not create a thread.")
...
endpoint("POST","/connectors/{connector}/heartbeat","Renew installation process ownership",record({"instance":string},["instance"]),security="connector",description="One random stable process identity owns a 90-second heartbeat lease. Reuse it across transient reconnects. A different process must wait for release or expiry.")
endpoint("POST","/connectors/{connector}/release","Release installation process ownership",record({"instance":string},["instance"]),security="connector",description="Call on graceful shutdown, after stopping command execution. Clears only the matching process heartbeat so a successor can reconnect immediately. Idempotent; a delayed release from an older process cannot disconnect its successor. Grants and command leases are unchanged. After an unclean exit the heartbeat expires after 90 seconds.")
