...
MESSAGE = """Alter the following code such that every time the user edits a message, let's make a versions field which has an encoded json object with timestamp keys and text values, where the value in the text field is just the value of the max(key) of the versions json object:
```
import os
import uuid
import hashlib
...
MESSAGE = "What is the coolest thing about minneapolis?"


def main():
    model = llama_cpp.Llama(
        model_path="/Users/ericflo/Development/finalechat/runtime/tmp-models/microsoft_Orca-2-7b/ggml-model-f16.gguf",
        chat_format="chatml",
        # chat_format="phind",
        # n_ctx=6144,
        n_ctx=512,
        ...
        messages=[
            {
                ...
                # "content": "‘MiniChat’是一个由‘Beccurio’开发的AI语言模型。下面是人类和MiniChat之间的一段对话。MiniChat的回复应当尽可能详细，并且以Markdown的形式输出。MiniChat应当拒绝参与违背伦理的讨论。</s>",
            },
            {
                "role": "user",
                "content": MESSAGE,
    ...
    for chunk in resp:
        print(
            chunk["choices"][0]["delta"].get("content", ""), sep="", end="", flush=True
        )
